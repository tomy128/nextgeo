...
import { Target, CheckCircle2, Lock, ArrowRight, Loader2, Mail, TrendingUp, ArrowUpRight } from 'lucide-vue-next'
...
const handleUnlock = async () => {
  ...
  try {
    const response = await $fetch<{ success: boolean; data: any }>('/api/leads', {
      ...
      body: {
        email: email.value,
        websiteUrl: store.formData.brandOrUrl || 'unknown'
      }
    })

    if (response.success && response.data?.id) {
      // Store the lead_id in a cookie that expires in 30 days
      const leadCookie = useCookie('lead_id', { maxAge: 60 * 60 * 24 * 30 })
      leadCookie.value = response.data.id
...
<template>
  <div class="py-16 md:py-24 relative">
    ...
    <!-- 锁定的覆盖层 (Paywall) -->
    <div v-if="!isUnlocked" class="absolute inset-x-0 -top-32 bottom-0 z-50 flex items-start justify-center pt-48">
      <!-- 渐变遮罩 -->
      <div class="absolute inset-0 bg-gradient-to-b from-transparent via-background/80 to-background pointer-events-none"></div>
      
      <div class="max-w-md w-full mx-4 glass-card rounded-3xl p-8 md:p-10 text-center shadow-glow shadow-accent/20 relative z-10 border border-accent/30">
        <div v-if="!submitSuccess">
          ...
          <h3 class="font-display text-2xl font-bold text-white mb-3">解锁深度归因与解决方案</h3>
          <p class="font-sans text-muted text-sm mb-8">输入企业邮箱，获取针对您品牌的定制化 AI 推荐提升指南、深度问题诊断及一键生成语料工作台。</p>
        ...
        <div v-else class="py-8 animate-fade-in-up">
          ...
          <p class="font-sans text-muted">正在为您展开深度报告与工作台...</p>
        </div>
      </div>
    </div>
